...
import { CreateCategoryModelMap } from './create-category.model-map';
import { MOCK_TOKEN } from '../../common/model-mock.decorator';

const { keys } = Object;

let categoryModel: CreateCategoryModelMap;
let validator: Validator;
let Mock: any;

test.before(() => {
  Mock = Reflect.getMetadata(MOCK_TOKEN, CreateCategoryModelMap);
...
test('should raise error when name is not defined', async t => {
  ...
  const model = new CreateCategoryModelMap({
    // @ts-ignore
    name: undefined,
    ...props,
  });
